...
I implemented a dry run option in `xsdm()`.
This is useful to check that the inputs are corrects and to test
how the dots parameters are handled.
For instance, check the `test-dots-in-xsdm.R` script to see how to deal with such cases.

```{r dryrun}
# initialize minimal input to see dry run
...
This can be used, for example, to check that the number of chains to run are correctly specified in `...`:

```{r dryrun-chains}
xsdm(r, p, "lewontin-cohen", chains = 123, dry = TRUE)
```
...
niche <- xsdm(
  list(scaled_r), p, "lewontin-cohen",
  warmup = 100, iter = 200, cores = 4, chains = 4, refresh = 10,
  dry = FALSE  # set to FALSE to run it
)
